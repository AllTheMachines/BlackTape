...
must_haves:
  ...
  artifacts:
    - path: "tools/test-suite/manifest.mjs"
      provides: "PHASE_25 export with 21 test entries, spread into ALL_TESTS"
      contains: "PHASE_25"
  key_links:
...
Purpose: Per project rules, test entries must be added after every phase. The PROCESS.md TEST-PLAN policy requires that test IDs defined in each PLAN.md's TEST-PLAN section are registered in the manifest. This plan creates all P25 test entries (P25-01 through P25-21).
...
```
  ...
  <done>
All 21 P25 test entries pass. Full code-only suite shows no new failures. `PHASE_25` export exists in manifest with 21 entries.
  </done>
</task>
...
This plan IS the test plan for the phase. The single task adds and runs all 21 P25 test entries.
...
- `node tools/test-suite/run.mjs --phase 25 --code-only` — all 21 P25 tests pass
- `npm run check` — TypeScript clean
</verification>

<success_criteria>
- PHASE_25 export exists in manifest.mjs with 21 entries (P25-01 through P25-21)

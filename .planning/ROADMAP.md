...
- [x] 32-03-PLAN.md — Release page Play Album + streamingLinks data

### Phase 33: Artist Claim Form
**Goal**: Artists can submit a claim request for their artist page via a /claim route, linked from every artist page; claims reach Steve via Cloudflare Worker + KV + email notification
**Depends on**: Phase 32
**Requirements**: CLAIM-01
**Success Criteria** (what must be TRUE):
  1. Every artist page shows an "Are you {Artist Name}? Claim this page" link near the artist name that navigates to /claim with the artist name pre-filled and read-only
  2. /claim route renders a form with fields: artist name (pre-filled + locked), email, message; validation blocks submission on invalid email or empty message
  3. Form submission POSTs to the Cloudflare Worker (/claim endpoint) which stores in KV and sends Steve a notification email
  4. On success, form is replaced by a warm confirmation message with a link back to the artist page
  5. The worker /admin endpoint lists claim submissions separately from contact messages
**Plans**: 2 plans
Plans:
- [ ] 33-01-PLAN.md — Cloudflare Worker: /claim endpoint + CORS fix for Tauri + /admin claim listing
- [ ] 33-02-PLAN.md — Mercury: /claim form page + artist page claim link

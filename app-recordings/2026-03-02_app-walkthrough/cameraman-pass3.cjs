/**
 * cameraman-pass3.cjs — Per-scene recording: artists + visual highlights
 *
 * Scenes:
  ...
  try {
    ...
    for (let i = 0; i < ARTISTS.length; i++) {
      ...
      await scene(`${idx}-artist-${a.slug}`, async () => {
        ...
        // Scroll through discography — the money shot
        await scroll(900, 32);
        await wait(400);
    ...
    for (let round = 0; round < 3; round++) {
      await scene(`style-map-${round + 1}`, async () => {
        ...
        // Wait for the SVG/canvas to render
        try {
    ...
    for (let g = 0; g < KB_GENRES.length; g++) {
      ...
      await scene(`kb-${genre}`, async () => {
        ...
        // Click a related genre link if visible
    ...
    await scene('discover-tags', async () => {
      ...
      // Click 2 tags to show intersection filtering
    ...
    await scene('player-bar-finale', async () => {
      // Navigate to a good artist page as backdrop
      await nav(`/artist/${ARTISTS[0].slug}`, 3000);
      await wait(500);

      // Slow sweeps across the player bar
